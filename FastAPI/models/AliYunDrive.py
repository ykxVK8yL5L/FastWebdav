...
class AliYunDrive():
    ...
    def __init__(self,provider='',refreshToken=''):
        ...
        try:
            with open("configs/aliyundrive.ini") as f:
                self.config.read_file(f)
        except IOError:
            # 如果配置文件不存在，创建一个空的配置文件
            with open("configs/aliyundrive.ini", "w") as f:
                self.config.write(f)
        ...
        if self.config.has_option(self.provider, 'drive_id'):
            self.driveId = self.config.get(self.provider, 'drive_id')
        else:
            self.get_drive_id()
    ...
    def get_drive_id(self) -> str:
        ...
        if drive_id == 'error':
            ...
            self.config.set(self.provider, 'drive_id',drive_id)
            with open('configs/aliyundrive.ini', 'w') as f:
                self.config.write(f)
            self.driveId = self.config.get(self.provider, 'drive_id')
